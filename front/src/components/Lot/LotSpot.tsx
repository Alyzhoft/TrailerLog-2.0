...
export default function LotSpot({ spot, trailers, lot, spotClicked, addOpen }: Props) {
	...
	if (trailers.find((trailer) => parseInt(trailer.spotNumber) === spot && trailer.trailerLocation === lot) !== undefined) {
		const trailer = trailers.find((trailer) => parseInt(trailer.spotNumber) === spot && trailer.trailerLocation === lot);
		if (trailer !== undefined) {
			return (
				<div>
					{editOpen ? <EditModal open={editOpen} close={() => setEditOpen(false)} trailer={trailer} spotNumber={spot} trailerLocation={lot} /> : <></>}
					...
					<div className="flex mx-1 w-8 h-28 bg-white rounded-md justify-center shadow-md border-gray-600 border-2">
						<button style={{ textOrientation: 'upright', writingMode: 'vertical-rl' }} className={classNames('text-black focus:outline-none w-full', `bg-${getColor(trailer)}`)} onClick={() => handleEditClick(trailer)}>
