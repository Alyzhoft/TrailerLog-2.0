	...
	return (
		<div style={screenHeight} className="flex flex-col justify-between h-screen mt-5">
			...
			<div className="hidden building:block">
				<Building dock={'RVAC'} doors={doors} trailers={trailers} spotClicked={(door) => setSpotClicked(door)} trailerClicked={(trailer: trailer) => setSelectedTrailer(trailer)} addOpen={() => setAddOpen(true)} tempModal={() => setTempModal(true)} />
			</div>
			<div className="building:hidden w-full h-full mt-5 ">
				...
				<div className="border-black border-t-2">
					<Lot spots={doors} lot={TrailerLocation.RVAC} trailers={trailers} trailerClicked={(trailer: trailer) => setSelectedTrailer(trailer)} spotClicked={(spot) => setSpotClicked(spot)} addOpen={() => setAddOpen(true)} tempModal={() => setTempModal(true)} />
				</div>
			</div>
		</div>
	);
